...
class MealieAPI {
  ...
  async getRecipe(slug) {
    return this.request(`/api/recipes/${slug}`);
  }
  ...
  /**
   * Add notes to an existing recipe
   *
   * @param {string} slug - The recipe slug
   * @param {Array} notes - Array of {title, text} objects
   * @returns {Promise<object>} - The updated recipe object
   */
  async addNotesToRecipe(slug, notes) {
    // First get the current recipe to preserve existing notes
    const recipe = await this.getRecipe(slug);
    const existingNotes = recipe.notes || [];

    // Merge new notes with existing
    const allNotes = [...existingNotes, ...notes];

    return this.patchRecipe(slug, { notes: allNotes });
  }
